...
Loads a PDF, indexes it into ChromaDB with Ollama embeddings,
...
def safe_build_vectorstore(chunks: list) -> Chroma:
    """Build a persisted ChromaDB vector store from document chunks."""
    logger.info("Clearing previous ChromaDB data at %s", CHROMA_DIR)
    ...
    try:
        ...
        raise AppError(
            ...
            "Try deleting the ./chroma_db folder and running again."
        ) from exc
...
def index_pdf(pdf_path: str) -> Chroma:
    ...
    return safe_build_vectorstore(chunks)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Agentic RAG — chat with your PDF using Ollama and LangChain.",
    )
    parser.add_argument(
        "pdf_path",
...
def main() -> None:
    """Application entry point: validate, index PDF, build agent, start chat."""
    args = parse_args()
    ...
    try:
        ...
        vectorstore = index_pdf(pdf_path)
        ...
        thread_id = str(uuid.uuid4())
        print("PDF indexed successfully. Starting chat...\n")
